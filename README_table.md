...
-   [MailboxLayer](https://mailboxlayer.com/): Provides 250 free validations per month.

```php
$provider = new \enricodias\EmailValidator\ServiceProviders\MailboxLayer('API_KEY');

$emailValidator->addProvider($provider, 'MailboxLayer');
```
...
|-|-|-|-|
| [Validator Pizza](https://www.validator.pizza/) | 120 verifications per hour | Free | ```isHighRisk()```, ```isRole()```, ```isFree()``` |
...
\* email suggestions are documented but as for now, the API never returns a suggestion.
